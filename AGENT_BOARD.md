...
|---|---|---|---|---|---|
| SESI-20260903-GALIH-HOST-AUDIT | Galih - XySpace Team | Host Engine | LAGI KERJA | Audit menyeluruh `host/`: `Disconnected` diberi masa tenggang 15 dtk (blip Wi-Fi tidak lagi mencabut pairing) + pelepasan slot kini menutup peer connection; satu sesi media per offer; inject 0x06 TEXT per batch + batas 4.096 unit; `set_streaming` tahan-poison; `GlobalFree` papan klip saat gagal; dua lint Windows-only diperbaiki. Pemeriksaan area: fmt + clippy(-D) + test Linux hijau, clippy windows-gnu hijau | 2026-09-03 |

## Antrean izin push

| ID Sesi | Agent | Ringkasan perubahan | Status izin | Disetujui oleh | Kapan | Run CI |
|---|---|---|---|---|---|---|
| SESI-20260903-GALIH-HOST-AUDIT | Galih - XySpace Team | Host Engine: (1) sesi tidak lagi dicabut saat `Disconnected` — masa tenggang 15 dtk sebelum slot dilepas, kebijakan dipindah ke `session::slot_action` (3 uji unit); (2) pelepasan slot kini MENUTUP peer connection (capture+encoder berhenti pasti; di Windows duplikasi DXGI yang menggantung bisa membuat sesi berikutnya layar hitam) dan dijaga `ControlState::stop_session_if_current` agar teardown sesi lama tidak menimpa keadaan/izin sesi baru (1 uji); (3) `offer`/renegosiasi dari client yang sama menutup sesi lama — satu sesi media per host; (4) 0x06 TEXT diinject per 32 karakter sambil mengikuti nilai kembali `SendInput`, dengan batas 4.096 unit UTF-16 yang dipotong di batas karakter (5 uji) — menutup item HANDOFF "kontrak keyboard" dari Laras; (5) `set_streaming` memakai `recover_lock` (satu-satunya `.lock().unwrap()` jalur produksi yang tersisa); (6) clipboard: `GlobalFree` bila `GlobalLock`/`SetClipboardData` gagal (sebelumnya bocor di heap global); (7) DUA lint Windows-only diperbaiki — `tool/check-host-windows.sh --clippy` MERAH di `main` sebelumnya dan tidak terlihat CI (job host-test memakai cfg non-Windows); (8) `host/Cargo.lock` disinkronkan 6.3.0 → 6.4.0; + CHANGELOG `[Belum terbit]` dan item HANDOFF (Client Flutter, CI/Release, Docs & Audit) | DISETUJUI | Xyckal (chat) | 2026-09-03 | dispatch Build diminta ke CI/Release via HANDOFF (operator: gas + dispatch); tanpa build otomatis dari push host (kebijakan 2dbd186); bukti lokal: `cargo fmt --check` hijau, `cargo clippy --all-targets -- -D warnings` hijau, `cargo test --all-targets` 75 unit + 1 loopback hijau (Linux), `tool/check-host-windows.sh --clippy` hijau (x86_64-pc-windows-gnu) |
