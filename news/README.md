...
2. Kirim email Resend ke seluruh `subscribers`. Pengirimnya memakai
   `EMAIL_FROM` bila dikonfigurasi; jika kosong, Worker memakai
   `auth@mail.xystudio.my.id`. Domain tersebut sudah diverifikasi di Resend.
   `onboarding@resend.dev` ditolak agar tidak dipakai untuk email produksi.
