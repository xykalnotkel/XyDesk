...
#[tauri::command]
fn start_engine(state: State<EngineState>, url: String, token: String) -> Result<EngineStatus, String> {
    let mut guard = state.0.lock().map_err(|e| e.to_string())?;
    if let Some(child) = guard.as_mut() {
        if child.try_wait().map_err(|e| e.to_string())?.is_none() {
            return Ok(EngineStatus { running: true, pid: Some(child.id()) });
        }
    }
    let exe = host_exe_path()?;
    ...
    let pid = child.id();
    *guard = Some(child);
    Ok(EngineStatus { running: true, pid: Some(pid) })
}
...
fn stop_engine(state: State<EngineState>) -> Result<EngineStatus, String> {
    ...
    Ok(EngineStatus { running: false, pid: None })
}
