...
ke signaling server Go dan ke aliran video WebRTC sungguhan — menggantikan
`RTCVideoView` placeholder.
...
| `lib/webrtc/rtc_service.dart` | RTCPeerConnection + data channel input + render |

## Dependensi tambahan (tambah ke pubspec.yaml)

```yaml
dependencies:
  flutter_webrtc: ^0.10.5   # WebRTC (mitra webrtc-rs / str0m di host)
...
final video = await rtc.startSession(
  signalingUrl: 'wss://xydesk-signaling.<kamu>.workers.dev/ws', // Cloudflare, gratis
  hostId: 'gaming-pc-01',
  token: '<token client>',
);
// tampilkan di widget:
...
STUN default = `stun.cloudflare.com` (gratis tanpa batas). TURN
(`turn.cloudflare.com`, 1 TB/bln gratis) ditambahkan bila koneksi direct gagal.
...
- Token: minta dari server (`signaling -issue <id>`), jangan hardcode.
