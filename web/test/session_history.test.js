...
test('preview URL remote dari storage tidak digunakan',()=>{const s=setup();s.values.set('xydesk.guest.history.v1',JSON.stringify([{...item(1),preview:'https://tracker.invalid'}]));assert.equal(s.loadGuestHistory()[0].preview,null);});

test('same device keeps manual preview across connections and renders one card',async()=>{
 const s=setup(),preview='data:image/jpeg;base64,/9j/2Q==';
 await s.saveSessionHistory({...item(1),preview},null);await s.saveSessionHistory({...item(2),endedAt:3},null);
 let rows=s.loadGuestHistory();assert.equal(rows.length,2);assert.equal(rows[0].preview,preview);assert.equal(s.deviceCards(rows).length,1);
