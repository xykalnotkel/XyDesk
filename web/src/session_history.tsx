...
export function SessionHistoryPage({renderReconnect}:{renderReconnect?:(item:HistoryItem)=>ReactNode}={}){
 ...
 {error&&<p role="alert">{error}</p>}{loading?<p role="status">Memuat riwayat…</p>:!items.length&&!error?<div className="history-empty"><h2>Belum ada sesi tersimpan</h2><p>Setelah sesi selesai atau terputus, detail PC muncul di sini. Wallpaper HD diambil otomatis saat diizinkan tanpa menangkap aplikasi terbuka dan dipakai ulang untuk ID yang sama.</p></div>:null}
 ...
 <header><div><h2>{reconnect.name}</h2><p>ID {reconnect.deviceId} · Masukkan password pairing. Password tidak disimpan di riwayat.</p></div><button className="btn ghost" type="button" aria-label="Tutup hubungkan lagi" onClick={closeReconnect}>×</button></header>
