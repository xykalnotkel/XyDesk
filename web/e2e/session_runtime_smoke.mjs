...
RtcSession.prototype.start = async function() {
 ...
 this.onMeta?.({displays:[],wanted:0,audio:{available:true,pipeline:'fixture'}});
 this.onMeta?.({displays:[],wanted:0,audio:{available:true,pipeline:'fixture'}});
...
await page.locator('.remote-control-cursor').waitFor({state:'visible'});
  ...
  await page.evaluate(()=>document.querySelector('.remote-control-cursor').style.transform='translate3d(-500px,-500px,0)');
  await page.waitForFunction(()=>!document.querySelector('.remote-control-cursor').style.transform.includes('-500'));
  assert.equal(await page.locator('.remote-control-cursor').getAttribute('data-revision'),'pointer-v2');
  assert.deepEqual(errors,[]);
  checks.push({viewport,defaultTrackpad:true,arrowVisible:true,swipe:true,tap:true,cancelNoClick:true,holdAndDrag:true,twoFingerScroll:true,directPositionBeforeClick:true,nativeFullscreenCursor:true,metadataRecovery:true,centerButton:true,audioPlayRetry:true,audioMute:true,savedPrefsSentOnce:true,randomUrl:true,pageErrors:errors});
  if(viewport.width===844)await page.screenshot({path:new URL('../../docs/qa/session-runtime-2026-09-17.png',import.meta.url).pathname});
  await context.close();
 }
 writeFileSync(new URL('../../docs/qa/session-runtime-2026-09-17.json',import.meta.url),JSON.stringify({result:'PASS',boundary:'Real React handlers, Chromium CDP touch events, local canvas stream; stubbed RtcSession transport, not Windows/Android injection',checks},null,2)+'\n');
